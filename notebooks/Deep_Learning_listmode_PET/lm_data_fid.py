...
class LMLinearOperator:
    ...
    def adjoint_ones(self) -> torch.tensor:
        if self._adjoint_ones is None:
            self._adjoint_ones =  self._A.T @ torch.ones(self._A.shape[0], device = self._A.device, dtype = torch.float64)
...
def test_lmlogl(dev: str, nx: int = 2, ny: int = 2):
    ...
    op = LMLinearOperator(6 * torch.rand(ny,nx, device = dev, dtype=torch.float64) + 24*torch.eye(ny,nx, device = dev, dtype = torch.float64))
