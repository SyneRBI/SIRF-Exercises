...
x_t = torch.tensor(lm_ref_recon.as_array(), device=dev, dtype=torch.float32, requires_grad=True).unsqueeze(0).unsqueeze(0)
...
#
res = torch.autograd.gradcheck(poisson_logL_grad_layer, (x_t, lm_obj_fun, initial_image, 0), 
                               eps=1e-3, atol=1e-4, rtol = 1e-3, fast_mode=True)