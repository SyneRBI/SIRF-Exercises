...
zero_operator_im2grad = op.ZeroOperator(image_dict['PET'], grad.range_geometry())
...
merge = MergeBlockDataContainers(cil.BlockGeometry(*[grad.range_geometry()]*2),[1/6,5/6])
merged_dgrad = op.CompositionOperator(merge, joint_dgrad)
G = fn.OperatorCompositionFunction(alpha* fn.SmoothMixedL21Norm(epsilon=1e-4), merged_dgrad)

maprl_synergistic = MAPRL(initial_estimate=initial_estimate, data_fidelity=F, prior=G,
                step_size=1, relaxation_eta=0.02, update_objective_interval=10)
maprl_synergistic.FOV_filter = bdc_FOV_filter()

maprl_synergistic.run(verbose=1, iterations=100)
